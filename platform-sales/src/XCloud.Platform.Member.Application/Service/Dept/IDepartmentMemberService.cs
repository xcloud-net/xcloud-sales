...
public interface IDepartmentMemberService : IXCloudApplicationService
{
    ...
    Task<Department[]> QueryAdminDepartmentsAsync(QueryAdminDepartmentsDto dto);
}

public class DepartmentMemberService : PlatformApplicationService, IDepartmentMemberService
{
    private readonly IMemberRepository<DepartmentAssign> _departmentMemberRepository;

    public DepartmentMemberService(IMemberRepository<DepartmentAssign> departmentMemberRepository)
    {
    ...
    public async Task SetAdminDepartmentAsync(JoinDepartmentDto dto)
    {
        ...
        if (ValidateHelper.IsNotEmptyCollection(dto.DepartmentIds))
        {
            ...
            {
                var assignment = new DepartmentAssign()
                {
                    DepartmentId = m,
                    AdminId = dto.AdminId,
    ...
    public async Task<SysAdmin[]> QueryDepartmentAdminAsync(IdDto dto)
    {
        ...
        var query = from deptAssign in db.Set<DepartmentAssign>().AsNoTracking()
    ...
    public async Task<Department[]> QueryAdminDepartmentsAsync(QueryAdminDepartmentsDto dto)
    {
        var db = await _departmentMemberRepository.GetDbContextAsync();
