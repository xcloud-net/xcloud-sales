...
public class SpecCombinationPriceService : SalesAppService, ISpecCombinationPriceService
{
        ...
        GoodsGradePriceAttachDataInput dto)
    {
        ...
        if (dto.GoodsInfo)
        {
            ...
            foreach (var m in data)
            {
                ...
                m.GoodsSpecCombination = this.ObjectMapper.Map<GoodsSpecCombination, GoodsSpecCombinationDto>(gradePriceGoods.combination);
    ...
    public async Task UpdateCombinationPriceAsync(UpdateGoodsPriceDto dto)
    {
        if (dto.Id <= 0 || dto.Price < 0)
            throw new ArgumentNullException(nameof(UpdateCombinationPriceAsync));

        var db = await this._gradePriceRepository.GetDbContextAsync();

        var combination = await db.Set<GoodsSpecCombination>().FirstOrDefaultAsync(x => x.Id == dto.Id);
        if (combination == null)
            throw new EntityNotFoundException(nameof(UpdateCombinationPriceAsync));

        if (combination.Price == dto.Price)
            return;

        var priceHistory = new GoodsPriceHistory()
        {
            GoodsId = combination.GoodsId,
            GoodsSpecCombinationId = combination.Id,
            PreviousPrice = combination.Price,
            Price = dto.Price,
            Comment = dto.Comment,
            CreationTime = this.Clock.Now
        };
        //add history
        db.Set<GoodsPriceHistory>().Add(priceHistory);
        //update combination price
        combination.Price = dto.Price;
        combination.LastModificationTime = priceHistory.CreationTime;

        await db.SaveChangesAsync();
    }

    public async Task UpdateCombinationCostPriceAsync(UpdateGoodsCostPriceDto dto)
    {
        if (dto.Id <= 0 || dto.CostPrice < 0)
            throw new ArgumentNullException(nameof(UpdateCombinationPriceAsync));
        ...
        //update combination price
        combination.CostPrice = dto.CostPrice;
